                ...
                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody id="fn_mark">   
                        ...
                        if (isset($students) && !empty($students)) {
                            ...
                            <?php $position = get_position_in_class($current_session_id, $current_class_id, @$result->avg_grade_point); ?>
                                <tr>
                                    ...
                                    <td>
                                        <select  class="form-control col-md-7 col-xs-12"  id="class_id_<?php echo $obj->id; ?>" name="promotion_class_id[<?php echo $obj->id; ?>]"  required="required" onchange="get_section_by_class(this.value, '', '<?php echo $obj->id ?>');">                                
                                    ...
                                    <td>
                                        <select  class="form-control col-md-7 col-xs-12" id="section_id_<?php echo $obj->id; ?>" name="promotion_section_id[<?php echo $obj->id; ?>]"  required="required">                                
                                            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>
                                            
                                            <?php if(isset($enroll) && $enroll->class_id == $current_class_id ){ ?>
                                                <?php foreach($current_class_sections as $sec1){ ?>
                                                    <option value="<?php echo $sec1->id; ?>" <?php if(isset($enroll) && $enroll->section_id == $sec1->id){ echo 'selected="selected"';} ?>><?php echo $sec1->name; ?></option>
                                                <?php } ?>                                          
                                            <?php }elseif(isset($enroll) && $enroll->class_id == $next_class_id){ ?>  
                                                <?php foreach($next_class_sections as $sec2){ ?>
                                                    <option value="<?php echo $sec2->id; ?>" <?php if(isset($enroll) && $enroll->section_id == $sec2->id){ echo 'selected="selected"';} ?>><?php echo $sec2->name; ?></option>
                                                <?php } ?>
                                            <?php }else{ ?>
                                                <?php foreach($next_class_sections as $sec2){ ?>
                                                    <option value="<?php echo $sec2->id; ?>" <?php if(isset($enroll) && $enroll->section_id == $sec2->id){ echo 'selected="selected"';} ?>><?php echo $sec2->name; ?></option>
                        ...
                        <?php }else{ ?>
                                <tr>
                                    <td colspan="13" align="center"><?php echo $this->lang->line('no_data_found'); ?></td>
                                </tr>
                        <?php } ?>
                    </tbody>
                </table>
...
<script type="text/javascript">
    ...
    function get_section_by_class(class_id, section_id, student_id){
         ...
         $.ajax({       
            ...
            success: function(response){                                                   
               if(response)
               {
                    $('#section_id_'+student_id).html(response);
               }
            }
        });  
